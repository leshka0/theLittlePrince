...
class App{
	...
	constructor(){
		...
		// AUDIO
		var audioFile = 'audio/loop/chapter7.mp3';
		...
		//AUDIO voice
		var audioFile = 'audio/voice/chapter7.mp3';
		var audioListener = new THREE.AudioListener();
		cameraUser.add( audioListener );
		...
		function growThorn() {
			...
			loader.load( 'models/chapter7/thorn.dae', function ( collada ) {
	...
	zoom( camera, zoom ){
		...
		cameraUser.position.set( 0, 0, 0 );
		cameraDev.position.set( 0, 0, 0 );
	...
	render( camera, left, bottom, width, height ){
		...
		if( !flags.debug ){
			//cameraUser.position.x = ( Math.sin(mouseX/100)*20 );
			//cameraUser.position.z = ( Math.cos(mouseX/100)*20 );
			//cameraUser.position.y += ( ( mouseY)/100 - cameraUser.position.y ) * .4;
			cameraUser.rotation.x += ( ( -mouseY)/1500 - cameraUser.rotation.x ) * .004;
			
			cameraUser.rotation.y -= ( ( mouseX)/160000);
		} 
		...
		lights.directional.intensity = Math.sin((cameraUser.rotation.y+0.7))/2 +0.5
		console.log(lights.directional.intensity)
		//lights.directional.position.x = Math.sin((counter2) )*150
